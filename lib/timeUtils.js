...
/**
 * Returns the strict start and end boundaries of the current reset cycle.
 * @param {string} dungeonId 
 * @param {Date} now 
 * @returns {{ lastReset: Date, nextReset: Date }}
 */
export function getResetWindow(dungeonId, now = new Date()) {
  const lastReset = getLastReset(dungeonId, now);
  const rule = DUNGEON_RESETS[dungeonId];
